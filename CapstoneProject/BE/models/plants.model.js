...
Plants.updateOne = (id, newPlantCName, newPlantLName, newPlantVitamins, newPlantMinerals,newPlantPharmaProps, newPlantIMGURL, NewPlantDesc, newPlantLike, result) => {
  ...
  if (newPlantLike){
    query += (comma?', ' : ' ')+`PlantLike=${newPlantLike}`;
  }
  query +=  ` WHERE id=${id}`
  console.log(query)
  ...
  sql.query(query, (err, res) => {
    ...
    console.log("plants: ", res);
    result(null, res);
    // console.log(newPlantLike)
  });
};

Plants.updateLike = (id, newPlantLike, result) => {
  let query = `update plants set PlantLike=${newPlantLike} WHERE id=${id}`;
